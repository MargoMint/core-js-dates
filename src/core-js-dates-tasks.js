...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.abs(newDateEnd - newDateStart) / oneDayInMilliseconds + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  return newDate >= newDateStart && newDate <= newDateEnd;
}
...
function formatDate(date) {
  ...
  const minutes = String(newDate.getUTCMinutes());
  const seconds = String(newDate.getUTCSeconds());
  const period = hours >= 12 ? 'PM' : 'AM';
  return `${month}/${day}/${year}, ${hours12}:${minutes.padStart(2, '0')}:${seconds.padStart(2, '0')} ${period}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  let allWeekendDays = 0;

  for (let i = 1; i <= nextMonth.getDate(); i += 1) {
    newDate.setDate(i);
